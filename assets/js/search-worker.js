// Web Worker for off-thread search processing
class SearchEngine {
  constructor() {
...
// Create a single SearchEngine instance for this worker
...
onmessage = function(e) {
  ...
  if (type === 'BUILD_INDEX') {
    engine.buildIndex(payload);
    postMessage({ messageId, type: 'INDEX_BUILT' });
  }

  if (type === 'SEARCH') {
    const results = engine.search(payload);
    postMessage({ messageId, type: 'SEARCH_RESULTS', results });
  }
};
