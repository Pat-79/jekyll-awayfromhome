...
# All variants of an image share the same aspect ratio (resize preserves it),
# so one recorded size per image is enough regardless of which variant a
# given <img> ends up referencing.
load_dimensions_file() {
  ...
  while IFS= read -r line; do
    if [[ "$line" =~ ^\"(.*)\":\ \{width:\ ([0-9]+),\ height:\ ([0-9]+)\}$ ]]; then
      key="${BASH_REMATCH[1]}"
      w="${BASH_REMATCH[2]}"
      h="${BASH_REMATCH[3]}"
      DIM_MAP["$key"]="${w}x${h}"
...
save_dimensions_file() {
...

  {
    ...
    printf '# to render width/height attributes without a build-time plugin.\n'
...
while IFS= read -r -d '' src; do
  ...
  dim_key="/${dir}/${stem}"
  dim_key="${dim_key//\/\//\/}"
  if [[ -z "${DIM_MAP[$dim_key]:-}" ]]; then
    # Upgrading an existing repo: back-fill dimensions for images processed by
    # an older version of this script, without forcing a full reprocess.
